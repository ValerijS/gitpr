...
// declaration  part:
    ...
    var count_of_nodes,
        ...
        cluster, //= [[node_i(name(number) of node(server), i - integer),  working_status(1 - working server, 0 - failure), {key_of_bucket_or_needle:Riak_object_value}, count_of_data_in_node, total_time_of_finish_performence_actions_at_node],...].
		clusterForKey,
		...
		k, n, r, rr, c, j, g, d, curr_ar, curr_nod, nod, change_gossip_protocol, node, node1;
...
//Initial setting part:
	...
	probOfGetNewKeyInWriteReqwest = 0.3;
	...
	clusterForKey = {};
	valueOfRiakObj = [];
	metaDataForKey = [['node_' + 0, 1, 0 +'_'+ 0]];
	clusterForKey  = {};
    for (k = 1; k <=  n_value; k++) {
        clusterForKey['node_' + k] = [ '0-0 is value = empty then: ', 0, [0, 0]];
        metaDataForKey.push(['node_' + k, 0, [0, 0]]);
    

	}
	

	gossip_protocol =[{start:[0, 0]}, {start:metaDataForKey}, {start:clusterForKey}, {start:[0, 0]}, {}, {start:['start']}, {start:[]}, {start:''}, {}, {}, {start:{node_1:['kuku']}}, {start:['history']}];
     cluster = [];
...
// iteration's part, beginning:
    ...
    w = 0;
	v = 0;
	gossip_protocol_repair = [['start', 0, 0.65]];
...
// iteration's part, write action:
        ...
        if (current_action === write_action) {//then do a realisation of  the action.
            w = 0;
            if (electOfEventWithProbability_p(probOfGetNewKeyInWriteReqwest)) {
               
                 list_of_entering_keys.push('k' + t + '_' + u);
                key_of_bucket_or_needle = 'k' + t + '_' + u;
                gossip_protocol[0][key_of_bucket_or_needle] = [];
                gossip_protocol[1][key_of_bucket_or_needle] = [['start', 0, 0]];
                ...
                gossip_protocol[10][key_of_bucket_or_needle] = {start:['']}; //{node_1:['beginning, then:', 0, [0, 0],0], node_2:['beginning, then:', 0, [0, 0],0]};
            ...
            while (j < n_value) {
			    ...
			    if(curr_nod[0]) {
                    ...
                    if (curr_nod[0][1]) {
					    ...
					    if (randomElection([1, 0], [1 - p_of_send_action * current_bucket_or_needle[3], p_of_send_action * current_bucket_or_needle[3]])) {
							...
							curr_nod[0][5] = Math.max(curr_nod[0][5], t) + use_time_for_performance_local_action_with_one_cond_unit_of_data * current_bucket_or_needle[4];//time for performance local action.
                           ...
                           // }
			                ...
			                total_used_resource = total_used_resource + (use_of_resource_by_write_action + use_of_resource_by_send_action) * current_bucket_or_needle[1]; //fresh information.
                            total_use_of_resource_by_write_action = total_use_of_resource_by_write_action + current_bucket_or_needle[1];
                            total_use_of_resource_by_send_action = total_use_of_resource_by_send_action + current_bucket_or_needle[1];
                       ...
                       // if(gossip_protocol[10][key_of_bucket_or_needle][curr_nod[0]]) {
                        ...
                        /* if(gossip_protocol[2][key_of_bucket_or_needle][curr_nod[0]]) {
					    gossip_protocol[10][key_of_bucket_or_needle][curr_nod[0][0]] = ['start: send_failure! value = ' + t +'_' + u +'-' + current_bucket_or_needle[0] + '-and, then: -' , curr_nod[0][5], [t, u], gossip_protocol[2][key_of_bucket_or_needle][curr_nod][0]][0] + '-pair: send_fall--' + t +'_' + u +'-' + current_bucket_or_needle[0] + '--and-'];
                         }*/ 
		                //c = c - 1;
			            total_count_of_failures = total_count_of_failures + 1; //fresh information.
                        count_of_failures_for_send_action = count_of_failures_for_send_action + 1;
				        total_used_resource = total_used_resource + use_of_resource_by_send_action * current_bucket_or_needle[1];
                        total_use_of_resource_by_send_action = total_use_of_resource_by_send_action + current_bucket_or_needle[1];
						 if(gossip_protocol[1][key_of_bucket_or_needle]) {
...
//main loop(oldKey)			
			...
			for (nod in gossip_protocol[2][key_of_bucket_or_needle]) {
               ...
               // }else{
                     //if(gossip_protocol[1][key_of_bucket_or_needle] && curr_nod[0] ) {
						//gossip_protocol[1][key_of_bucket_or_needle].push([curr_nod[0][0], 25, t + '_' + u]);
								//}                    
	               // c = c - 1;
		        }
		...
		for (buck in gossip_protocol[2][key_of_bucket_or_needle]) {
		    ...
		    if (randomElection([1, 0], [1 - p_of_send_action * current_bucket_or_needle[3], p_of_send_action * current_bucket_or_needle[3]])) {
				...
				if (curr_meta_value[0]) {
					...
					total_used_resource = total_used_resource + (use_of_resource_by_read_action + use_of_resource_by_send_action) * current_bucket_or_needle[1]; //fresh information.
                    total_use_of_resource_by_read_action = total_use_of_resource_by_read_action + current_bucket_or_needle[1];
                    total_use_of_resource_by_send_action = total_use_of_resource_by_send_action + current_bucket_or_needle[1];
